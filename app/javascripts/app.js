...
function createProjectTableRow(projectId, amountNeeded, deadline, amountRaised, state) {
            ...
            .then(function (id) {
                   fundingHub.getProjectDetails.call( id, {from: account})
                    .then(function(values) {
                      ...
                      var amountNeeded = Number(values[1]);
                      var deadline = Math.round(((Number(values[2]) * 1000) - Date.now()) / (1000*60*60));
                      var amountRaised = Number(values[3]);
                      var projectState = Number(values[4]);
                      createProjectTableRow(projectId, amountNeeded, deadline, amountRaised, projectState);
                    }).catch(function(e) {
                      console.log(e);
